...
async function loadLessonSource(source) {
  ...
  throw new Error('TranslateToBase lesson source must be a lesson path string or lesson data object.');
}
...
async function loadLessonsFromSources(sources) {
  ...
  for (const source of list) {
    ...
    try {
      ...
      console.warn('Unable to load previous lesson for TranslateToBase exercise.', error);
    }
  }
...
function selectVocabForExercise(vocabEntries) {
  if (!vocabEntries.length) {
    throw new Error('TranslateToBase lesson does not contain any vocabulary entries.');
  }

  if (vocabEntries.length < 2) {
    throw new Error('TranslateToBase requires at least two vocabulary entries to build a multiple-choice exercise.');
  }
  ...
  const distractors = shuffleArray(remaining).slice(0, Math.min(3, remaining.length));
...
function collectCorrectAnswersFromChoices(choices) {
  ...
  if (!answers.size) {
    throw new Error('TranslateToBase exercise could not determine a correct answer.');
  }
...
function createExerciseConfigFromEntry(entry, overrides = {}) {
  const baseInstructions = 'Tap the English meaning that matches the Sinhala word.';
  ...
  return {
    ...
    successMessage:
      overrides.successMessage || `Great job! “${entry.si}” ${transliteration ? `(${transliteration}) ` : ''}means “${english}”.`,
...
async function resolveLessonSpecification(configSource) {
  ...
  if (!resolved || typeof resolved !== 'object') {
    throw new Error('TranslateToBase requires a lesson path string or configuration object.');
  }
  ...
  if (!lessonSource) {
    throw new Error('TranslateToBase config must include a lessonPath or lesson data.');
  }
  ...
  const overrides = {};
  ['instructions', 'successMessage', 'errorMessage', 'initialMessage', 'mascot', 'mascotAlt'].forEach((key) => {
    if (key in resolved && resolved[key] !== undefined) {
      overrides[key] = resolved[key];
    }
  });

  return { lessonSource, previousSources, overrides };
}
...
function createExerciseLayout(config, meta = {}) {
  ...
  if (meta.isNewWord) {
    ...
    badge.className = 'translate-to-base__new-word';
  ...
  instructions.textContent = config.instructions || 'Tap the English meaning that matches the Sinhala word.';
